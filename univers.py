...
class Univers(object):
    ...
    def plot_x_with_time(self):
        from pylab import figure,legend,show
        
        figure(self.name)
        for agent in self.population :
            agent.plot_X_with_time(self.time)
        
        legend()
        show()

    def plot_y_with_time(self):
        from pylab import figure,legend,show
        
        figure(self.name)
        for agent in self.population :
            agent.plot_Y_with_time(self.time)
...
if __name__=='__main__':
    ...
    P_fixe_2 = Particule(p0=V3D(monUnivers.dimensions[0]/2 +10,monUnivers.dimensions[1]/2 ),fix=True)
    P_osc = Particule(p0=V3D(monUnivers.dimensions[0]/2,monUnivers.dimensions[1]/2 - 10))
    P_osc_parallel = Particule(p0=V3D(monUnivers.dimensions[0]/2 +10,monUnivers.dimensions[1]/2 - 10))
    P_osc_2 = Particule(p0=V3D(monUnivers.dimensions[0]/2,monUnivers.dimensions[1]/2 - 15), color = (0,0,0))
    ...
    ressort = SpringDamper(P_fixe,P_osc,k=1000,l0=10, c=1)
    ressort_2 = SpringDamper(P_osc,P_osc_2, k = 1000, l0 = 5, c =1)
    ressort_3 = SpringDamper(P_fixe_2,P_osc_parallel,k=1000,l0=10, c=1)
    ressort_4 = SpringDamper(P_osc,P_osc_parallel,k=10,l0=10, c=1)
    
    
    monUnivers.addParticule(P0,P_fixe,P_osc,P_osc_2, P_fixe_2 , P_osc_parallel)
    monUnivers.addGenerators(force,boing,boing2,ressort,ressort_2,ressort_3, ressort_4)
    
    def myInteraction(self,events,keys):
        Force_deuxieme_pendule = 10 
        # controle de leader avec le clavier
        if keys[ord('z')] or keys[pygame.K_UP]: # And if the key is z or K_DOWN:
            P_osc.applyForce(V3D(0,150))
...
# Surcharge de la fonction ici
    ...
    monUnivers.plot()
    monUnivers.plot_x_with_time()
    monUnivers.plot_y_with_time()
